...
def getsource(sourcetype, sourcesize, k=1):
  ...
  if sourcetype == 'random':  # Gaussian iid source, mu = 0, sigma = 1
    ...
    x[nonz] = np.random.randn(sparsity, 1)  # get randomly the value for the non zero elements
...
def getmatrix(sourcesize, matrixtype, measurementsize, conditionnumber=1):
  ...
  if matrixtype == 'random':  # Gaussian iid matrix, mu = 0, sigma = 1
    a = np.random.randn(measurementsize, sourcesize)  # sensing matrix
     ...
     a = np.random.randn(measurementsize, sourcesize)  # random well conditioned matrix
     u, s, v = np.linalg.svd(a)  # get the svd decomposition
     nsv = min(sourcesize, measurementsize)  # number of sv
     s[np.nonzero(s)] = np.linspace(conditionnumber, 1, nsv)  # modify the sv to make cond(A) = conditionnumber
...
def getmeasurements(a, x, noisetype, var=1, outlierproportion=0):
  ...
  if noisetype == 'none':  # noiseless case
    ...
    n = var * np.random.randn(measurementsize, 1)  # additive Gaussian noise
    p = np.random.permutation(measurementsize)
    noutliers = np.round(outlierproportion * measurementsize)  # how many measurements are outliers
    outindex = p[0:noutliers]  # getting random indexes for the outliers
    n[outindex] = np.var(y) * 10 * np.random.randn(noutliers, 1)  # the outliers have a variance ten times larger than clean data
...
def weights(u, kind, lossfunction, clipping, nmeasurements):
  ...
  if kind == 'M':  # if M-estimator
    if lossfunction == 'huber':  # with Huber loss function
      z = scorefunction(u, 'huber', clipping)  # call the huber score function
      w = np.zeros(u.shape)
      i = np.nonzero(u)
      w[i] = z[i] / (2 * u[i])  # definition of the weights for M-estimator
    elif lossfunction == 'squared':  # with square function
      z = scorefunction(u, 'squared', clipping)  # call the ls score function
      w = np.zeros(u.shape)
      i = np.nonzero(u)
      w[i] = z[i] / (2 * u[i])
...
def mscaleestimator(u, tolerance, b, clipping, kind):
  ...
  while np.abs(rho_old) > tolerance and k < maxiter:
    delta = rho_old / np.mean(scorefunction(u / s, kind, clipping) * u / s) / s
    isqu = 1
    ok = 0
    while isqu < 30 and ok != 1:
      ...
      if np.abs(rho_new) < np.abs(rho_old):
        ...
        isqu += 1
      if isqu == 30:
        maxiter = k  # we tell it to stop, but we keep the iter for info
      rho_old = rho_new
      k += 1
  return np.abs(s)
...
def getinitialsolution(y, a):
  ...
  while k < 100:
    ...
    if r == n:  # we assume that in these cases asubsample is well condtitioned
      initialx = inv.leastsquares(ysubsample, asubsample)  # use it to generate a solution
      return initialx
    else:
      k += 1
      if k == 100:
        sys.exit('I could not find initial solutions!')  # die gracefully
